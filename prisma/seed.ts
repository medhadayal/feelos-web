...
async function main() {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    await prisma.dailyPlan.upsert({
      ...
      update: { date: new Date(), focusItems: ["Plan morning", "Deep work", "Sync with team"], selfCare: "5-minute breathing" },
      create: { userId: user.id, date: new Date(), focusItems: ["Plan morning", "Deep work", "Sync with team"], selfCare: "5-minute breathing" },
    });
